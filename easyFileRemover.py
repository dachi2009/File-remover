import os
import subprocess
'''I made this tool for learning purposes only and this tool is small project and i didnt even plan to make it big so once i finish with it i will leave it like that'''
...
'''code beginning and global variables'''
...
is_secure_user_input = input("enter y/n: ").strip()
if is_secure_user_input.lower() == "y":
    is_secure = True
else:
    is_secure = False

'''Main function'''
file_path_input = input("enter full path e.g /path/to/your/file no ~/: ")
def Removing_file(file_path, secure=False):
    '''Checking at first if it exists'''
    if not os.path.exists(file_path):
        print("Cant find path {}".format(file_path))
        return 0
    
    '''list of dangerous files you can delete from'''
    
    dangerous_files = ["/", "/root", "/dev", "/bin", "/boot",
                        "/etc", "/var", "/tmp", "/sysroot", "/sys", "/sbin",
                        "/srv", "/run", "/lib", "/media", "/lib64", "/ostree", "/mnt",
                        "/usr", "/proc", "/opt"]
    
    '''deletation from dangerous directories logic begins here'''
    ...
    if is_dangerous:
        print("[!] DANGER: ARE YOU SURE YOU WANT TO DELETE FILES FROM THIS PATH? {}".format(file_path), "ITS VERY DANGEROUS AND MAY BREAK SYSTEM")
    ...
    while True:
        ...
        '''used it second time so after deleeting first one it it can display this message and no need to restart script'''
        
        if not os.path.exists(file_path):
            print("[!] either file doesnt exists or it has already been deleted")
            break
        
        '''logic of choosing what to do with selected file'''
        
        print("[?] last warning are you sure you would like to delete file {}".format(file_path))
        
        user_input_danger = input("enter here (y/n): ").strip()
        
        '''if user decides to still delete from dangerous directory'''
        
        if user_input_danger.lower() == "y":
            print("[!] DANGEROUS BUT IT'S USER'S DECISION...")
            
            '''deleteing via shredding'''
            ...
            if secure:
                ...
                if os.path.isfile(file_path):
                    ...
                    print("Shredding was successfull")
                else:
                    print("[!] can not shred a whole directory\nskipping...")
                
                '''choosing what to do with file'''    
                
                print('[?] choose which one to use')
                print('[?] must be either \n1) rm \neither\n2) /dev/null method\nadditional\n3) Exit')
                
                '''deletation begins here'''
                
                while True:
                    print("1) which is not recommeneded as its deleting file and simple deleting makes file recoverable\n2) is recommended as it nullifies file",
                        "it means it will completely wipe off data also nullifying means sending data to /dev/null which makes data dissapear forever but in this srcipt\n it cant nullify with /dev/null since it can make zero-days appear when doing that method in python\n",
                        "3) is used to exited the program")
                    ...
                    if user_input == "1":
                        ...
                        if not os.path.isfile(file_path):
                            print("[!] ERROR: {} is a directory, can't remove one".format(file_path))
                            print("[!] change it to file")
                            return
                        with open(file=file_path, mode='w') as e:
                            pass
                ...
                while True:
                    ...
                    if user_input_2 == "1":
                        ...
                        print('checking if file was successfully deleted...')
                        if not os.path.exists(file_path):
...
Removing_file(file_path=file_path_input, secure=is_secure)
